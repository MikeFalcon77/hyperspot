...
status: accepted
date: 2026-05-13
decision-makers: hyperspot core team
---
...
* Need a valid FIPS 140-3 claim on macOS for outbound TLS in `cf-server`.
...
Chosen option: **Option B — custom rustls `CryptoProvider`**, packaged as `cf-rustls-corecrypto-provider`.
...
* hyperspot owns and maintains a ~1500 LOC crypto-plumbing crate (`libs/rustls-corecrypto-provider`). Every primitive must be unit-tested against NIST CAVS / RFC vectors.
* TLS 1.2 + TLS 1.3 with FIPS-approved cipher suites only: AES-128/256-GCM, P-256/P-384 ECDHE, ECDSA + RSA-PSS + RSA PKCS#1 v1.5. ChaCha20, x25519, ED25519 are **out of scope** by design.
...
* The provider only supports client-side TLS. Server-side `KeyProvider` is a rejecting stub — acceptable because hyperspot terminates HTTPS at the reverse proxy in production.
...
* Unit-level: `cargo test -p cf-rustls-corecrypto-provider` (113 tests; ≥95% line coverage; NIST CAVS / RFC 4231 / RFC 5869 vectors).
* Integration: `tests/handshake_smoke.rs` performs real TLS 1.2 + TLS 1.3 handshakes against `openssl s_server`.
* Wire-level: `examples/cf-fips-probe` against `https://www.howsmyssl.com/a/check` must report only FIPS-approved cipher suites and named groups in `given_cipher_suites` / `given_named_groups`.
* Runtime: `vmmap <pid>` of a hyperspot binary doing outbound TLS must show `/usr/lib/system/libcorecrypto.dylib` loaded.
...
* Bad, because hyperspot now owns ~1500 LOC of crypto-plumbing code; maintenance and audit responsibility falls on us.
...
* Bad, because users running cf-server on macOS in deployments that require FIPS would have to switch OS — not a viable customer ask.
...
* Wire-level smoke: [`examples/cf-fips-probe`](../../../examples/cf-fips-probe) — see its `README.md` for the four-layer verification chain.
...
- **Related ADR**: [`0005-fips-feature-target-conditional-shim`](0005-fips-feature-target-conditional-shim.md) — explains how the FIPS `Cargo` feature picks the corecrypto path on macOS without poisoning the build with aws-lc-fips.
