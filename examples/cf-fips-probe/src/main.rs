//! cf-fips-probe — outbound HTTPS smoke for the modkit / modkit-http FIPS path.
//!
//! Boots the same crypto stack that cf-server uses (`modkit::bootstrap::
...
//!
//! ```sh
//! cargo run -p cf-fips-probe                              # non-FIPS build
//! cargo run -p cf-fips-probe --features fips              # FIPS-conformant build
//! cargo run -p cf-fips-probe --features fips -- --url https://example.com
//! ```
