...
Boots the **same crypto stack** that `cf-server` uses
...
verify a FIPS build end-to-end without spinning up cf-server or writing
ad-hoc test code.
...
| Linkage | `otool -L` shows only Apple frameworks (no `aws-lc-fips`) | `otool -L target/.../cf-fips-probe` |
...
cargo build -p cf-fips-probe

# FIPS-conformant build
cargo build -p cf-fips-probe --features fips
```
...
```sh
cargo run -p cf-fips-probe --features fips -- \
...
cargo run -p cf-fips-probe --features fips -- --url https://localhost:8443/
```
...
```sh
cargo test -p cf-rustls-corecrypto-provider --test handshake_smoke
```
...
```sh
otool -L target/debug/cf-fips-probe
```
...
workspace-feature-unification shim (`cf-rustls-fips-shim`) has regressed —
file a bug.
...
- The probe verifies the **client** path. Server-side TLS is intentionally
  unsupported in `cf-rustls-corecrypto-provider` (hyperspot terminates
