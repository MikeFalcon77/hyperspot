...
impl PaymentApi for PaymentApiGrpcService {
    async fn charge(
        ...
        let req: api_contracts_sdk::models::ChargeRequest = request.into_inner().into();
    ...
    async fn list_payments(
        ...
        let ctx = require_security_context(request.metadata())?;
        let filter: api_contracts_sdk::models::ListPaymentsFilter =
...
/// (`x-modkit-problem-bin`, per [`modkit_transport_grpc::attach_problem`]).
///
...
fn canonical_to_status(err: CanonicalError) -> Status {
    ...
    if let Err(attach_err) =
        modkit_transport_grpc::attach_problem(status.metadata_mut(), &problem)
    {
