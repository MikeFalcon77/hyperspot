...
async fn grpc_get_invoice_not_found_returns_canonical_error() {
    ...
    let err = PaymentApi::get_invoice(&client, anonymous_ctx(), bogus)
        .await
        .unwrap_err();
    // The status maps to NOT_FOUND on the wire, which the canonical-errors
    // bridge surfaces as an Internal-with-detail (404 has no direct
    // CanonicalError variant). Either way it is an error.
    let _: CanonicalError = err;
}
