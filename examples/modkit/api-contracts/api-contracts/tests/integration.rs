...
//! produced by `#[modkit::rest_contract]`. The local transport stays a
...
use modkit_contract::policy::PolicyStack;
...
async fn http_charge_then_get_invoice() {
    ...
    let invoice =
        PaymentApi::get_invoice(&client, test_ctx(), charge_resp.payment_id.to_string())
            .await
            .unwrap();
    assert_eq!(invoice.payment_id, charge_resp.payment_id);
    assert_eq!(invoice.amount_cents, 1000);
...
async fn http_get_invoice_not_found() {
    ...
    let err = PaymentApi::get_invoice(
        &client,
        test_ctx(),
    ...
    .await
    .unwrap_err();
    // The server returns `Problem` with status 404; the SDK's
    // `From<TransportError>` impl maps it onto an `Internal` variant for
    // simplicity in this demo — the round-trip assertion is that we see an
...
async fn http_list_payments_sse() {
    ...
    let items: Vec<_> = PaymentApi::list_payments(&client, test_ctx(), ListPaymentsFilter::default())
        .collect::<Vec<_>>()
        .await
        .into_iter()
        .collect::<Result<Vec<_>, _>>()
...
async fn client_hub_resolves_http_client() {
    ...
    let client_hub = Arc::new(ClientHub::new());
    let http_client: Arc<dyn PaymentApi> = Arc::new(PaymentApiRestClient::new(
