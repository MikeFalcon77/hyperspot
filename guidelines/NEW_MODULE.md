...
| REST error mapping             | `impl From<DomainError> for Problem`      | `src/api/rest/error.rs`    | Centralize RFC-9457 mapping via `From` trait; `Problem` implements `IntoResponse` directly.                                                  |
| Handler return type            | `ApiResult<T, DomainError>`               | `src/api/rest/handlers.rs` | Use `ApiError::from_domain(e)` for error conversion; type aliases simplify signatures.                                                       |
| OpenAPI responses registration | `.error_400(openapi)`, `.error_404(...)` | `src/api/rest/routes.rs`   | Register error statuses using convenience methods on `OperationBuilder`.                                                                     |
...
impl From<crate::domain::error::DomainError> for UsersInfoError {
    fn from(e: crate::domain::error::DomainError) -> Self {
        ...
        match e {
            DomainError::UserNotFound { id } => Self::NotFound(id),
            DomainError::EmailAlreadyExists { email } => Self::EmailConflict(email),
...
- **All API methods MUST accept `&SecurityCtx`** for authorization and tenant isolation.
...
4. **`src/contract/client.rs`:**
   ...
   **Rule:** All methods MUST accept `&SecurityCtx` as the first parameter for authorization and access control.
...
#[modkit::module(
    ...
    client = crate::contract::client::MyModuleApi, // Path to native client trait
...
2. **`src/module.rs` - `impl Module for YourModule`:**
   ...
   **Rule:** The `init` function is the composition root. It MUST:
   ...
   6. Register the native client in ClientHub: `ctx.client_hub().register::<dyn UsersInfoApi>(api);`
...
impl Module for UsersInfo {
    async fn init(&self, ctx: &ModuleCtx) -> anyhow::Result<()> {
        ...
        // Create local client adapter and register in ClientHub
        let local_client = UsersInfoLocalClient::new(domain_service);
        let api: Arc<dyn UsersInfoApi> = Arc::new(local_client);

        // Register directly in ClientHub (no expose_* helper needed)
        ctx.client_hub().register::<dyn UsersInfoApi>(api);
        info!("UsersInfo API registered in ClientHub");
        Ok(())
    }
...
The local client simply forwards the `SecurityCtx` to service methods.
...
1. **`src/gateways/local.rs`:**
   ...
   **Rule:** Pass `SecurityCtx` directly to service methods — the local client is a thin wrapper.
...
pub mod contracts {
    pub use mymodule_contracts::*;
}
pub mod grpc {
    pub use mymodule_grpc::*;
}

use crate::contracts::{MyModuleApi, MyModuleError};
use crate::grpc::{MyModuleService, MyModuleServiceServer, SERVICE_NAME};

/// Module struct
#[modkit::module(
    name = "my_module",
    capabilities = [grpc],
    client = crate::contracts::MyModuleApi
)]
pub struct MyModule {
...
- [examples/modkit/users_info/](../examples/modkit/users_info/) — Reference implementation of a local module
