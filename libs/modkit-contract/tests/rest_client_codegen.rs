//! End-to-end test for `#[modkit::rest_contract]` REST client codegen.
//!
...
use modkit_contract::runtime::config::{ClientConfig, RetryConfig};
use modkit_contract::runtime::transport_error::TransportError;
use modkit_canonical_errors::{CanonicalError, Problem};
use modkit_contract::{contract, rest_contract};
...
async fn projection_trait_is_implementable_for_generated_client() {
    ...
    let client = std::sync::Arc::new(DemoApiRestClient::new(ClientConfig::new(base_url)));

    let _as_base: std::sync::Arc<dyn DemoApi> = client.clone();
    let _as_projection: std::sync::Arc<dyn DemoApiRest> = client.clone();
    ...
    let via_projection = DemoApiRest::echo_get(&*_as_projection, anonymous_ctx(), "abc".to_owned())
        .await
        .unwrap();
    let via_base = DemoApi::echo_get(&*_as_base, anonymous_ctx(), "abc".to_owned())
        .await
        .unwrap();
...
async fn unary_get_round_trip() {
    let (base_url, _) = start_server().await;
    let client = DemoApiRestClient::new(ClientConfig::new(base_url));
...
async fn retryable_recovers_after_transient_failure() {
    ...
    let client = DemoApiRestClient::new(cfg);
...
async fn streaming_with_reconnect_config_happy_path() {
    ...
    let cfg = ClientConfig::new(base_url).with_sse_reconnect(
        modkit_contract::runtime::config::ReconnectConfig::enabled(
            3,
