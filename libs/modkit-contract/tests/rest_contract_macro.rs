//! Behavior tests for `#[modkit::rest_contract]`.
//!
...
use modkit_contract::{
    HttpFieldBinding, HttpMethod, contract, rest_contract, validate_contract,
...
mod fakes {
    ...
    impl FakeSecurityContext {
        /// Mirrors the shape of `modkit_security::SecurityContext::bearer_token`
...
pub enum FakeError {
    ...
    #[error("transport: {0}")]
    Transport(#[from] modkit_contract::runtime::transport_error::TransportError),
}
...
#[contract(module = "billing", version = "v1")]
...
fn get_with_path_param_emits_path_binding() {
    let binding = billing_api_rest_http_binding();
    let get_invoice = binding
        .find_method("get_invoice")
...
#[contract(module = "stream-svc", version = "v1")]
