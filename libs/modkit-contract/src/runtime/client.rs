...
use modkit_http::RequestBuilder;
use serde::Serialize;
...
/// Add a JSON body to a request builder. Wraps `modkit_http`'s fallible
...
/// delay each attempt, capped at `max_delay`. No jitter — reconnect storms
/// from a single client are not the workload we're optimising for.
...
mod tests {
    ...
    // accepts. `modkit_http::HttpClient::new()` spawns the buffered tower
    // service on the current Tokio runtime, so this has to be a
    // `#[tokio::test]`.
    #[tokio::test]
    async fn with_json_body_compiles_and_chains() {
        let client = modkit_http::HttpClient::new()
            .expect("default modkit-http build is infallible in standard env");
        let req = client.post("https://example.invalid/test");
        let _ = with_json_body(req, &serde_json::json!({ "k": "v" })).expect("json body");
    }
}
