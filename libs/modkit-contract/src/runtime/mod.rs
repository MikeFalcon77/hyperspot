...
//! contract types without pulling in `modkit-http` / `tokio` transitively.
//!
...
//! `modkit-contract-runtime` crate would add Cargo / CI ceremony without
...
//! standalone (without `modkit-contract`'s IR + macros). Until that
