...
pub fn validate_grpc_binding(
    ...
    let contract_methods: HashSet<&str> = contract.methods.iter().map(|m| m.name.as_str()).collect();
    ...
    for method_binding in &binding.methods {
        ...
        let contract_method = contract
            ...
            .expect("binding/contract methods reconciled above");
        let kind_streaming = matches!(contract_method.kind, super::contract::MethodKind::ServerStreaming);
...
mod tests {
    ...
    fn rejects_duplicate_rpc_name() {
        ...
        let errs = validate_grpc_binding(&sample_contract(), &binding).unwrap_err();
        assert!(errs.iter().any(|e| e.message.contains("duplicate rpc_name")));
    }
    ...
    fn empty_package_or_service_rejected() {
        ...
        let errs = validate_grpc_binding(&sample_contract(), &binding).unwrap_err();
        assert!(errs.iter().any(|e| e.message.contains("package must not be empty")));
    }

    #[test]
    fn proto_variant_mapping() {
        assert_eq!(GrpcIdempotency::NoSideEffects.proto_variant(), "NO_SIDE_EFFECTS");
