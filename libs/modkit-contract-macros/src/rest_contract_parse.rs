...
pub fn parse(attr: RestContractAttr, item: ItemTrait) -> syn::Result<RestContractModel> {
    ...
    for trait_item in &item.items {
        ...
        let key = (parsed.http_method.ir_variant().to_owned(), parsed.path_template.clone());
...
fn parse_method(method: &TraitItemFn) -> syn::Result<RestMethodModel> {
    ...
    let result_types = Some(parse_return_type(&method.sig.output, method.sig.ident.span())?);
