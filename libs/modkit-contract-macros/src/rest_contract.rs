//! Code generation for `#[modkit::rest_contract]`.
//!
//! Emits two artifacts:
...
fn generate_client_struct(model: &RestContractModel, support: &TokenStream) -> TokenStream {
    ...
    let doc = format!(
        "Generated REST client for [`{}`].\n\nProduced by `#[modkit::rest_contract]`.",
        model.trait_ident
    );
    ...
    quote! {
        ...
        pub struct #client_ident {
            http: ::modkit_http::HttpClient,
            config: #support::runtime::config::ClientConfig,
        }

        #[cfg(feature = "rest-client")]
        impl #client_ident {
            /// Build a new client with a default `modkit-http` HTTP client.
            ///
            /// This `new` is **infallible** so the caller can write
            /// `Arc::new(FooApiRestClient::new(cfg))` and register directly
            /// in `ClientHub` without `?` plumbing. The default
            /// `modkit-http` build is infallible in any standard environment
            /// (it does not perform network I/O or load FIPS-only ciphers).
            /// For caller-controlled HTTP client construction (custom TLS,
            /// FIPS, mTLS), use [`Self::with_http_client`] or
            /// [`Self::try_new`].
            #[must_use]
            pub fn new(config: #support::runtime::config::ClientConfig) -> Self {
                let http = Self::build_default_http_client()
                    .expect("modkit-http default HttpClient builder is infallible in standard env; \
                             use ::with_http_client / ::try_new for custom HTTP client builds");
                Self { http, config }
            }

            /// Build a new client, propagating any error from the underlying
            /// `modkit-http` client construction (e.g. FIPS / custom-TLS init).
            ///
            /// # Errors
            /// Returns whatever `modkit_http::HttpClient::builder().build()` returned.
            pub fn try_new(
                config: #support::runtime::config::ClientConfig,
            ) -> ::std::result::Result<Self, ::modkit_http::HttpError> {
                let http = Self::build_default_http_client()?;
                Ok(Self { http, config })
            }

            /// Build the default `modkit-http` HttpClient used by `new`/`try_new`.
            ///
            ...
            fn build_default_http_client() -> ::std::result::Result<
                ::modkit_http::HttpClient,
                ::modkit_http::HttpError,
            > {
                ::modkit_http::HttpClient::builder()
                    .retry(::std::option::Option::None)
                    .transport(::modkit_http::TransportSecurity::AllowInsecureHttp)
                    .build()
            }

            /// Build a new client with a caller-supplied `modkit-http`
            /// HTTP client.
            #[must_use]
...
fn generate_client_method(
    ...
    let fields_init = build_fields_json(method);
    ...
    // `modkit-http`'s `.json()` is fallible (returns `Result<RequestBuilder,
    ...
    quote! {
        async fn #method_ident #sig {
            ...
            let __attempt = || async {
                // `modkit-http` has no `.bearer_auth()` helper — use the
...
fn build_fields_json(method: &RestMethodModel) -> TokenStream {
    let entries = method.params.iter().filter_map(|p| {
        ...
        Some(quote! {
            __obj.insert(
                #key.to_owned(),
                ::serde_json::to_value(&#ident).unwrap_or(::serde_json::Value::Null),
            );
        })
    });

    quote! {
        let __fields: ::serde_json::Value = {
