...
use syn::{
    Attribute, Data, DataEnum, DeriveInput, Fields, Ident, LitStr, Meta, Variant,
};
...
pub fn generate(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let problem_path = quote! { ::modkit_canonical_errors::Problem };
    let category_path = quote! { ::modkit_canonical_errors::ProblemCategory };
...
fn parse_variant(variant: &Variant, enum_attrs: &EnumAttrs) -> syn::Result<ParsedVariant> {
    ...
    let code = code.ok_or_else(|| {
        syn::Error::new(
            variant.span(),
            "variant requires `#[error_code(\"...\")]`",
        )
    })?;
    let domain = domain.or_else(|| enum_attrs.default_domain.clone()).ok_or_else(
        || syn::Error::new(
            variant.span(),
    ...
    match &v.fields {
        VariantFields::Unit => {
            ...
            quote! {
                    ...
                    ::modkit_canonical_errors::Problem::contract_error(
