//! Code generation for `#[modkit::grpc_contract]`.
//!
...
use crate::projection::{
    ...
    render_method_inputs, render_method_return_ty, rewrite_streaming_signature,
    strip_method_attrs, type_path_ends_with,
};
...
/// Mirrors `modkit-contract-protogen`'s synthesized-request convention so
    ...
    let impls: Vec<TokenStream> = model
        ...
        .filter_map(|method| {
            let wire_params: Vec<&GrpcParam> = method
                ...
                .iter()
                .filter(|p| {
                    p.ident != "self" && !type_path_ends_with(&p.ty, "SecurityContext")
                })
                .collect();
            // Exactly one wire param of a proto-direct primitive type.
            let [param] = wire_params.as_slice() else { return None };
            if !is_proto_direct_primitive(&param.ty) {
                return None;
            }
            let request_ty_ident = format_ident!(
                "{}Request",
                method.ident.to_string().to_upper_camel_case()
            );
...
/// implements [`modkit::GrpcRepr`]. The guard fires at trait-definition time
...
fn generate_repr_guards(model: &GrpcContractModel, support: &TokenStream) -> TokenStream {
    ...
    let secctx_guard = if secctx_asserts.is_empty() {
        ...
        // `modkit_contract::grpc_repr`). The default impl for
        // `modkit_security::SecurityContext` is gated on `grpc-client` —
...
fn generate_binding_fn(model: &GrpcContractModel, support: &TokenStream) -> TokenStream {
    ...
    let method_entries = model.methods.iter().map(|m| build_method_binding(m, support));
...
fn generate_client_method(
    ...
    let stubs = &model.stubs_module;
    // Mirror `modkit-contract-protogen`'s naming convention: the proto
    ...
    let body_ident = match body_param_ident(method) {
        Some(b) => b,
        None => {
    ...
    if method.server_streaming {
        return generate_streaming_client_method(
            method, stubs, support, &rpc_method_ident, &sig_inputs, &return_ty,
            &body_ident, &ctx_ident, ok_ty, err_ty,
        );
    }
        ...
        async fn #method_ident #sig_inputs #return_ty {
            let __inner = || async {
                ...
                let __response = __client
                    ...
                    .await
                    .map_err(#support::grpc::map_tonic_status)?;
