...
/// Resolve the path that user code uses to refer to the `modkit_contract`
/// crate. Falls back to `::modkit::contract_support` when the contract crate
/// is reachable only through the umbrella `modkit` re-export.
