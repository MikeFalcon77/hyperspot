...
//! elsewhere by `modkit-contract::openapi`. The same `&[(name, Schema)]`
...
use modkit_contract::ir::contract::{ContractIr, FieldIr, MethodIr, PrimitiveType, TypeRef};
use modkit_contract::ir::grpc::{GrpcBindingIr, GrpcIdempotency};
...
impl std::fmt::Display for ProtoGenFeature {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ProtoGenFeature::OptionOfVec => {
                f.write_str("Option<Vec<_>> nested optional/repeated")
            }
...
/// `modkit_contract::openapi::generate_openapi_spec`: a list of
...
pub fn generate_proto_file(
    ...
    for binding_method in &binding.methods {
        ...
        let input = method_input_type(contract_method, &mut messages, lock)?;
    ...
    for (name, def) in messages.iter_mut() {
        let entry = lock.messages.entry(name.clone()).or_default();
        let current: BTreeSet<String> = def.fields.iter().map(|f| f.name.clone()).collect();
    ...
    for (name, def) in enums.iter_mut() {
...
fn method_input_type(
    ...
    let wire_fields: Vec<&FieldIr> = method
        ...
        .fields
        .iter()
        .filter(|f| !is_security_context_field(f))
        .collect();
...
fn is_security_context_field(field: &FieldIr) -> bool {
    if field.name == "ctx" {
        return true;
    }
    matches!(&field.ty, TypeRef::Named(name) if name == "SecurityContext")
}
...
fn translate_schema(
            ...
            .is_some_and(|t| t == "string")
    {
        ...
        enums.insert(
            ...
            EnumDef { variants, ..EnumDef::default() },
        );
        return Ok(());
    }
    ...
    {
        enums.insert(schema_name.to_owned(), EnumDef { variants, ..EnumDef::default() });
        return Ok(());
    }
    ...
    if let Some(branches) = schema.get("oneOf").and_then(|v| v.as_array())
        && let Some(oneof) = collect_externally_tagged_oneof(schema_name, branches, enums, queue)?
    {
        messages.insert(
            schema_name.to_owned(),
            MessageDef {
    ...
    if schema.get("type").and_then(|v| v.as_str()).is_some_and(|t| t == "object") {
        ...
        // stability).
        let mut sorted_keys: Vec<&str> = properties.keys().map(|s| s.as_str()).collect();
        sorted_keys.sort();
...
fn json_schema_to_proto_field(
    ...
    ) && schema.get("$ref").is_none()
    {
        ...
        extra_enums
            ...
            .or_insert(EnumDef { variants, ..EnumDef::default() });
    ...
    let normalized_type: Option<&str> = if let Some(arr) = schema.get("type").and_then(|v| v.as_array()) {
...
fn collect_externally_tagged_oneof(
    ...
    for (idx, branch) in branches.iter().enumerate() {
        ...
        let key = required.into_iter().next().unwrap();
        ...
        let shape =
            json_schema_to_proto_field(schema_name, &key, payload_schema, false, queue, extra_enums)?;
...
fn render_proto(
    ...
    out.push_str("// GENERATED by modkit-contract-protogen — DO NOT EDIT BY HAND.\n");
    out.push_str("// Regenerate via the SDK's `gen_grpc_proto` example.\n\n");
    out.push_str("syntax = \"proto3\";\n");
    out.push_str(&format!("package {package};\n\n"));
    out.push_str(&format!(
        "// {service} — module: {module}, version: {version}\n",
        service = service,
    ...
    for m in methods {
        ...
        out.push_str(&format!("{leading_comment}\n"));
        out.push_str(&format!(
        ...
        out.push_str(&format!("    option idempotency_level = {level};\n"));
        out.push_str("  }\n");
    }
    out.push_str("}\n\n");
    ...
    for (name, def) in messages {
        ...
        if let Some(oneof) = &def.oneof {
            out.push_str(&format!("  oneof {label} {{\n", label = oneof.label));
        ...
        if !def.reserved_numbers.is_empty() {
            let nums: Vec<String> = def.reserved_numbers.iter().map(|n| n.to_string()).collect();
            out.push_str(&format!("  reserved {};\n", nums.join(", ")));
        }
        if !def.reserved_names.is_empty() {
            let names: Vec<String> = def.reserved_names.iter().map(|n| format!("\"{n}\"")).collect();
            out.push_str(&format!("  reserved {};\n", names.join(", ")));
        }
        out.push_str("}\n\n");
    }
    ...
    for (name, def) in enums {
        ...
        out.push_str(&format!("  {sentinel} = 0;\n"));
        ...
        let mut numbered: Vec<(String, u32)> = def
            ...
            .map(|v| {
                ...
                let num = numbers
                    ...
                    .expect("every variant in EnumDef must have been pre-assigned via the lockfile");
                (shouty, num)
            })
            .collect();
        numbered.sort_by_key(|(_, n)| *n);
        for (shouty, num) in numbered {
            out.push_str(&format!("  {shouty} = {num};\n"));
        }
...
mod tests {
    use super::*;
    use modkit_contract::ir::contract::{
        FieldIr, Idempotency, InputShape, MethodIr, MethodKind, ServiceIr, TypeRef,
    };
    use modkit_contract::ir::grpc::{GrpcBindingIr, GrpcIdempotency, GrpcMethodBindingIr};

    fn sample_contract() -> ContractIr {
        ServiceIr {
            name: "PaymentApi".into(),
            module: "service-hub-demo".into(),
            version: "v1".into(),
    ...
    fn synthesizes_request_for_multi_param_methods() {
        ...
        contract.methods[0].input.fields = vec![
            FieldIr {
                ...
                ty: TypeRef::Primitive(modkit_contract::ir::contract::PrimitiveType::String),
                optional: false,
            },
            FieldIr {
                name: "include_history".into(),
                ty: TypeRef::Primitive(modkit_contract::ir::contract::PrimitiveType::Bool),
                optional: false,
            },
        ];
        let proto = generate_proto_file(
            &contract,
            &sample_binding(),
            &[("ChargeResponse", resp_schema()), ("PaymentStatus", status_schema())],
            &mut ProtoLockfile::empty(),
        )
        .expect("generated");
        assert!(proto.contains("message ChargeRequest"));
    ...
    fn unknown_referenced_type_errors() {
        ...
        assert!(matches!(
            err,
            ProtoGenError::UnknownTypeReference { .. }
        ));
    }
}
