...
use modkit_contract_protogen::{ProtoLockfile, generate_proto_file};

fn sample_contract() -> modkit_contract::ir::contract::ContractIr {
    ServiceIr {
...
fn adding_field_does_not_shift_existing_numbers() {
    ...
    .unwrap();
    let baseline_amount = *lock.messages["ChargeRequest"].fields.get("amount_cents").unwrap();
    let baseline_currency = *lock.messages["ChargeRequest"].fields.get("currency").unwrap();
...
fn removing_field_moves_to_reserved() {
    ...
    .unwrap();
    let removed_number = *lock.messages["ChargeRequest"].fields.get("description").unwrap();
