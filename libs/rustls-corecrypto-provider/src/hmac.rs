...
// SAFETY: `HmacKey` stores three POD fields (`CCHmacAlgorithm` is a `u32`,
// `CCHmacContext` is a fixed-size `[u32; 96]`, `tag_len` is a `usize`); no
// pointers, no thread-affine state. `sign_concat` only takes `&self` and
// clones `self.template` byte-for-byte into a local; `CCHmacUpdate` /
// `CCHmacFinal` are called on that local clone, never on `&self.template`,
// so concurrent calls cannot race on the C state. The underlying CommonCrypto
// HMAC primitives do not touch process-wide state.
unsafe impl Send for HmacKey {}
unsafe impl Sync for HmacKey {}
...
impl Key for HmacKey {
    fn sign_concat(&self, first: &[u8], middle: &[&[u8]], last: &[u8]) -> Tag {
        ...
        let mut out = [0u8; 48]; // max of SHA-256 (32) and SHA-384 (48)
