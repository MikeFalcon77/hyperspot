...
* Good, because no additional authentication surface — `llm_provider` inherits the `SecurityContext` from `chat_service` in-process
...
* Good, because no security surface — no ports, no auth, no TLS between services
